package com.beverages.factory;

import static com.beverages.Utility.Constants.MILK;
import static com.beverages.Utility.Constants.MINT;
import static com.beverages.Utility.Constants.SODA;
import static com.beverages.Utility.Constants.SUGAR;
import static com.beverages.Utility.Constants.WATER;

import java.util.*;
import java.util.stream.Collectors;

import com.beverages.dataLoading.BeveragesContentAndPrices;
import com.beverages.exception.DuplicateIngredientException;
import com.beverages.exception.IllegalIngredientsException;
import com.beverages.exception.InvalidOrderException;

public class BeverageFactory {

	//      This stores the combination of Items with allowed exclusions
	Map<String, List<String>> beveragesMap = BeveragesContentAndPrices.getBeveragesMap();
	...
	private Double calculateInvoice(List<String> itemWithIngredients) {
		...
		if (itemWithIngredients.size() > 1)
			for (int i = 1; i < itemWithIngredients.size(); i++) {
				ingredientsValue = ingredientsValue + getIngredientRates().get(itemWithIngredients.get(i));
			}
